...
- Committed reusable suite: `573d7c7`, 27 additional offline smoke tests. Strict live smoke correctly fails on the separate `pi-better-openai` load error. Full bridge extension health is not yet verified.
- Pi LCM native load: original ABI failure reproduced, rebuild completed, Node 24 in-memory SQLite check passed. This isn't a claim that every already-running session reinitialized LCM.
