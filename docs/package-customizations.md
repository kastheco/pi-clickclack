...
| `pi-lcm` → `better-sqlite3` | Dependency `11.10.0`, rebuilt for Node `24.20.0`, ABI `137`; native crash under investigation | In-memory SQLite load passes, but the full smoke process aborts in `Statement::~Statement` with a Node cleanup-hook assertion. No LCM source or database edits yet. | Rebuild fixed the ABI mismatch, not the native lifecycle crash. Don't claim LCM healthy or perform another runtime cutover until the full process exits successfully. Keep Node versions aligned. |
...
A rollback of the LCM native binary must match the Node runtime too. Restoring the Node 22 binary alone into the Node 24 bridge recreates the original failure. A package rollback isn't permission to roll back conversation data or discard newer turns.
...
- Committed reusable suite: `573d7c7`; pre-execution tool guard `acdfd0d`. The final Node 24.20.0 offline run passed 162 tests. Fresh extension diagnostics are empty after the Codex compatibility patch. Live smoke remains blocked by the LCM native crash, not the former Codex loader error.
- Pi LCM native load: original ABI failure reproduced, rebuild completed, Node 24 in-memory SQLite check passed. Full process later failed `node::RemoveEnvironmentCleanupHook` with `(env) != nullptr`; a null exit-code task receipt isn't success. Offline reproduction and lifecycle diagnosis are underway.
